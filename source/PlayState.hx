...
class PlayState extends MusicBeatState
{
	...
	var vegethEngineWatermark:FlxText;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			{	
				...
				john.frames = Paths.getSparrowAtlas('warzone/tankmanKilled1');
		...
		// Add Vegeth Engine watermark
		vegethEngineWatermark = new FlxText(4, healthBarBG.y
			+ 50, 0,
			SONG.song
			+ " - "
			+ CoolUtil.difficultyFromInt(storyDifficulty)
			+ (Main.watermarks ? " | VE " + MainMenuState.vegethEngineVer : ""), 16);
		vegethEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		vegethEngineWatermark.scrollFactor.set();
		add(vegethEngineWatermark);

		if (PlayStateChangeables.useDownscroll)
			vegethEngineWatermark.y = FlxG.height * 0.9 + 45;
		...
		vegethEngineWatermark.cameras = [camHUD];
		if (loadRep)
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			if (luaModchart.getVar("showOnlyStrums", 'bool'))
			{
				...
				vegethEngineWatermark.visible = false;
			...
			else
			{
				...
				vegethEngineWatermark.visible = true;
	...
	if(SONG.song.toLowerCase() == 'senpai' && FlxG.keys.justPressed.NINE)
		{
			if (iconP1.animation.curAnim.name == 'bf-old-pixel')
				iconP1.animation.play(SONG.player1);
			else
				iconP1.animation.play('bf-old-pixel');
		}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		switch (daRating)
		{
			...
			case 'sick':
				...
				if (!curStage.startsWith('schoolEvilB'))// IT WONT WORK ON THORNS IF NOT
				{
